...
function process_content() {
    ...
    $('#content p:visible').find('img').each(function() {
	if (parseInt($(this).height()) > 400 )
	    $(this).css("height", "400px");
    });

    $('#content div:visible').find('center').each(function() {
	...
	if ($(this).find('img').length){
		...
		if (para_offset > abs_container_bottom) {
		    ...
		    obscured_para = $(this).parent();
		    console.log( $(this).html() );
		    return false;
		}
		last_unobscured_para = $(this).parent();
	    });
	}else{
    ...
    */
    ...
    if (obscured_para != null && obscured_para.find('img').length) {
	var html = obscured_para.html();
	obscured_para.html('<p>' + '' + '</p>');
	obscured_para.after('<p class="img">'+ html +'</p>');
	// Hide all the remaining paragraphs
	obscured_para.nextAll('p').hide();
	obscured_para.hide();
    }
    else if (obscured_para != null && obscured_para.text() != '') {
